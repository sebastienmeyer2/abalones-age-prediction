...
predict_error <- function(lm,x_test) {
  return(residual_mean_of_squares(x_test$rings, predict(lm, x_test)))
}
```
...
We will now try out the following model: $\log(Y) = X \beta + \epsilon$ where $X$ contains the square of **height** plus **height2**.
...
anova(lm_log_height, lm_log_height2)
```
...
```{r}
residual_mean_of_squares(abalone_test$rings, predict_log_to_rings(lm_log_height,abalone_test))
residual_mean_of_squares(abalone_test$rings, predict_log_to_rings(lm_log_height2,abalone_test))
```
...
Finally, all the coefficients are significant in this model, and the final RSS on test data is below 5.
...
lm_complete <- lm(
    ...
    diameter + log_diameter + diameter * adulthood + log_diameter * adulthood + height + I(height^2) +
    log_height + height * adulthood + I(height^2) * adulthood + log_height * adulthood + shucked_weight
...
```
We can see that assumption 2,3 and 4 are not satisfied. Only assumption 1 is satisfied.
...
summary(rf_complete_log)
predict_log_error(rf_complete_log, abalone_test)
```
...
```{r}
set.seed(42)
# Random Forest with selected features (log)
rf_multi_log <- randomForest(
    log_rings ~ length * adulthood + log_length + height + height * adulthood +
    shucked_weight * adulthood + log_shucked_weight * adulthood + viscera_weight +
    log_whole_weight * adulthood + whole_weight * adulthood + log_shell_weight,
    data = abalone_train,
...
summary(rf_multi_log)
predict_log_error(rf_multi_log, abalone_test)
```
...
```{r}
set.seed(42)
# XGBoost with all computed features(log)
xgb_complete_log <- train(
    log_rings ~ adulthood + length + log_length + length * adulthood + log_length * adulthood +
    diameter + log_diameter + diameter * adulthood + log_diameter * adulthood + height + height2 +
    log_height + height * adulthood + height2 * adulthood + log_height * adulthood + shucked_weight
    + log_shucked_weight + shucked_weight * adulthood + log_shucked_weight * adulthood +
...
summary(xgb_complete_log)
predict_log_error(xgb_complete_log, abalone_test)
```
```{r}
set.seed(42)
...
summary(xgb_multi_log)
predict_log_error(xgb_multi_log, abalone_test)
```
...
summary(glm_complete_log)
predict_log_error(glm_complete_log, abalone_test)
```
...
```{r}
set.seed(42)
# glmnet with selected features (log)
glm_multi_log <- train(
    log_rings ~ length * adulthood + log_length + height + height * adulthood +
    shucked_weight * adulthood + log_shucked_weight * adulthood + viscera_weight +
    log_whole_weight * adulthood + whole_weight * adulthood + log_shell_weight,
...
summary(glm_multi_log)
predict_log_error(glm_multi_log, abalone_test)
```
We see that glmnet method could get a slight better result for full model glm_multi. 
