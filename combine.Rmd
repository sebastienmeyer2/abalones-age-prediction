...
We plot the graph of $\log(Rings)$ against $Height$, $\log(Rings)$ against $\log(Height)$, and $\sqrt{\log(Rings)}$ against $\log(Height)$
...
```{r}
summary(model_ancova_2_1)
plot(model_ancova_2_1)
ncvTest(model_ancova_2_1)
acf(residuals(model_ancova_2_1),main="Auto-correlation plot")
durbinWatsonTest(model_ancova_2_1)
shapiro.test(residuals((model_ancova_2_1)))
```
```{r}
train_error1 <- function(lm) mean(lm$residuals^2)
round_to_integer1 <- function(number){
  if (number - floor(number) > 0.5) {
    return(ceiling(number))
  }
  return(floor(number))
}
back_to_Ring1 <- function(vector){
  as.double(lapply(vector,round_to_integer))
}
loss1 <- function(y, y_predict){
   return(mean((y -y_predict)^2))
  #return(mean(abs(y -y_predict)))
}
predict_as_ring1 <- function(lm, X_test){
  return( back_to_Ring1(predict(lm,X_test)))
}
predict_error1 <- function(lm,X_test) {
  return(loss1(X_test$Rings, predict_as_ring1(lm, X_test) ) )
}
```
...
summary(rf1)
rf2=randomForest(Rings~Height,abalone_train,importance=TRUE)
summary(rf2)
rf3=randomForest(Rings ~Length*Sex+Diameter*Sex+Height*Sex +Shucked_weight*Sex+ Viscera_weight*Sex+Shell_weight *Sex+ Whole_weight*Sex,abalone_train,importance=TRUE)
rf4=randomForest(Rings ~Sex+Length+Diameter+Height +Shucked_weight+ Viscera_weight+Shell_weight + Whole_weight,abalone_train,importance=TRUE)
rf5=randomForest( sqrt_log_Ring ~  log_Length+log_Diameter+log_Height*Sex +log_Shucked_weight*Sex+ log_Viscera_weight*Sex+log_Shell_weight *Sex+ log_Whole_weight*Sex,data_1,importance=TRUE)
rf6=randomForest( sqrt_log_Ring ~  log_Length+log_Diameter+log_Height*Sex +log_Shucked_weight*Sex+ log_Viscera_weight*Sex+log_Shell_weight *Sex+ log_Whole_weight*Sex,data_2,importance=TRUE)
rf7=randomForest( sqrt_log_Ring ~ log_Height + log_Shell_weight*Sex + log_Whole_weight*Sex + log_Diameter,data_1,importance=TRUE)
```


```{r}
predict_error1(rf1,abalone_test)
predict_error1(rf2,abalone_test)
predict_error1(rf3,abalone_test)
predict_error1(rf4,abalone_test)
predict_error(rf5,abalone_test)
predict_error(rf6,abalone_test)
predict_error(rf7,abalone_test)
```

```{r}
library(caret)
importance1 <- varImp(rf3, scale=FALSE)
print(importance1)
importance2 <- varImp(rf4, scale=FALSE)
print(importance2)
```
...
options(warn=-1)
model1 <- train(
  Rings ~., data = abalone_train, method = "xgbTree",
  trControl = trainControl("cv", number = 10)
  )
model1$bestTune
predict_error1(model1,abalone_test)
```


```{r}
model2 <- train(
  Rings ~Height, data = abalone_train, method = "xgbTree",
  trControl = trainControl("cv", number = 10)
  )
model2$bestTune
predict_error1(model2,abalone_test)
```


```{r}
model3 <- train(
  Rings ~Length+Diameter*Sex+Height*Sex +Shucked_weight*Sex+ Viscera_weight*Sex+Shell_weight *Sex+ Whole_weight*Sex, data = abalone_train, method = "xgbTree",
  trControl = trainControl("cv", number = 10)
  )
model3$bestTune
predict_error1(model3,abalone_test)

model4 <- train(
  Rings ~Length+Diameter+Height +Shucked_weight+ Viscera_weight+Shell_weight + Whole_weight, data = abalone_train, method = "xgbTree",
  trControl = trainControl("cv", number = 10)
  )
model4$bestTune
predict_error1(model4,abalone_test)

model5 <- train(
  sqrt_log_Ring ~  log_Length+log_Diameter+log_Height*Sex +log_Shucked_weight*Sex+ log_Viscera_weight*Sex+log_Shell_weight *Sex+ log_Whole_weight*Sex,data=data_1, method = "xgbTree",
  trControl = trainControl("cv", number = 10)
  )
model5$bestTune
predict_error(model5,abalone_test)

model6 <- train(
  sqrt_log_Ring ~  log_Length+log_Diameter+log_Height*Sex +log_Shucked_weight*Sex+ log_Viscera_weight*Sex+log_Shell_weight *Sex+ log_Whole_weight*Sex,data=data_2, method = "xgbTree",
  trControl = trainControl("cv", number = 10)
  )
model6$bestTune
predict_error(model6,abalone_test)

model7 <- train(
  sqrt_log_Ring ~ log_Height + log_Shell_weight*Sex + log_Whole_weight*Sex + log_Diameter,data=data_1, method = "xgbTree",
  trControl = trainControl("cv", number = 10)
  )
model7$bestTune
predict_error(model7,abalone_test)
```
...
g1 <- train(
  Rings ~., data = abalone_train, method = "glmnet",
  trControl = trainControl("cv", number = 10),
  tuneLength = 10,
  )
g1$bestTune
predict_error1(g1,abalone_test)
```


```{r}
g2 <- train(
  Rings ~Height*Sex, data = abalone_train, method = "glmnet",
  trControl = trainControl("cv", number = 10)
  )
g2$bestTune
predict_error1(g2,abalone_test)
```
...
g3$bestTune
predict_error1(g3,abalone_test)
...
g4$bestTune
predict_error1(g4,abalone_test)
...
library(extraTrees)
et1 <- train(
  Rings ~., data = abalone_train, method = "extraTrees",
  trControl = trainControl("cv", number = 10)
  )
et1$bestTune
predict_error1(et1,abalone_test)
```
```{r}
et2 <- train(
  Rings ~Height, data = abalone_train, method = "extraTrees",
  trControl = trainControl("cv", number = 10)
  )
et2$bestTune
predict_error1(et2,abalone_test)
```
